...
describe "RecordSynonymTest_Omar" do
  ...
  it "should return false since the synonym is empty" do
    id = word.id
    result, synonym = Synonym.record_synonym_full_output("",id)
    expect(result).to eq (false)
  end

  it "should return false for the seconde synonym because it's a duplicates" do
    id = word.id
    result, synonym = Synonym.record_synonym("معنى",id)
    result2, synonym2 = Synonym.record_synonym_full_output("معنى",id)
    expect(result).to eq (true)
    expect(result2).to eq (false)
  end

  it "should return false since there is no such keyword" do
    result, synonym = Synonym.record_synonym_full_output("ليس",1000)
    expect(result).to eq (false)
  end

  it "should return false since the synonym is not in arabic" do
    id = word.id
    result, synonym = Synonym.record_synonym_full_output("test",id)
    expect(result).to eq (false)
  end

  it "should return true since all conditions are met" do
    id = word.id
    result, synonym = Synonym.record_synonym_full_output("عربية",id)
    expect(result).to eq (true)

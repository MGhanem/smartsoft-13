...
describe Vote, vote_test: true do
    ...
    describe "record_vote" do
      ...
      it "never record a vote for a non existing gamer" do
        success, vote_instance = Vote.record_vote(1000000000, sTwo.id)
        success.should eq(false)
        vote_instance.errors.messages.should eq({:gamer_id=>["this gamer_id doesn't exist"]})
        success_again, vote_instance_again = Vote.record_vote(g.id, 100000000000)
        success_again.should eq(false)
        vote_instance_again.errors.messages.should eq({:synonym_id=>["this synonym_id doesn't exist"]})
      end
    end
